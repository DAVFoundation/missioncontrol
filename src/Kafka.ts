...
const config: IConfig = {
  kafkaSeedUrls: [process.env.KAFKA_HOST || 'localhost:9092'],
};

export default {
  createTopic: (topicId: string) => kafka.createTopic(topicId, config),
  sendMessage: (topicId: string, message: string) =>
    kafka.sendMessage(topicId, message, config),
  sendPayloads: (payloads: ProduceRequest[]) =>
    kafka.sendPayloads(payloads, config),
