...
export class DroneChargingProvider extends BaseProvider {
  private protocol = 'drone_charging';
  protected tableName = 'providers_drone_charging';
  ...
  public async save(provider: IDroneChargingProvider): Promise<boolean> {
  ...
  public async query(need: INeed): Promise<IDroneChargingProvider[]> {
    const cassandra: Cassandra = await Cassandra.getInstance();
    ...
    const providers: IDroneChargingProvider[] = [];
